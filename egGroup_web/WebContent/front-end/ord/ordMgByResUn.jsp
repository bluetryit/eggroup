...
<% 
	    ...
	    for (FeastInfoVO feastInfoVO : feaList) {
	    	...
	    	List<OrdVO> ordList = ordSvc.getAllUnOrdByFea(feastInfoVO.getFea_no());//拿出該飯局的所有訂單VO
				  ...
				  <div class="card">
				    <div class="card-header" id="heading${feastVO}">
				      <h2 class="mb-0">
				        ...
				        <td>飯局訂單總金額：${ordSvc.getAllUnOrdTwdByFea(feastVO)}</td>
				        </tr></table>
				        </button>
				        <table><tr>
				        <td><FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front-end/ord/ord.do">
			     			<input type="submit" value="接受訂單">
			    			<input type="hidden" name="fea_no"  value="${feastVO}">         
...
<c:if test="${AcceptSuccess.equals(\"true\")}" var="flag" scope="session">
<%session.removeAttribute("AcceptSuccess");%>
	<script>
		$(function() {
			alert("已成功接受訂單！");
		});
...
<c:if test="${RejectSuccess.equals(\"true\")}" var="flag" scope="session">
<%session.removeAttribute("RejectSuccess");%>
	<script>
		$(function() {
			alert("已成功拒絕接受訂單！");
		});
